...
-- + Inspect frame — Inspired by ElvUI, adapted to MythicHub style
-- Fully compatible with WoW 12.x (TWW / Midnight)
-- =====================================

MythicHub_CharacterSkin = MythicHub_CharacterSkin or {}
local CS = MythicHub_CharacterSkin

-- =====================================
-- LOCALS & CACHES
-- =====================================

local ADDON_FONT       = STANDARD_TEXT_FONT or "Fonts\\FRIZQT__.TTF"
local ADDON_FONT_BOLD  = UNIT_NAME_FONT or STANDARD_TEXT_FONT or "Fonts\\FRIZQT__.TTF"
local ADDON_TEXTURE    = "Interface\\Buttons\\WHITE8x8"
local L = MythicHub_L

local isInitialized   = false
local inspectSkinned  = false

-- Colors matching MythicHub's dark theme
...
local ACCENT_COLOR       = { 0.00, 0.686, 1.00, 1 }
...
local function GetSettings()
    return MythicHubDB and MythicHubDB.characterSkin or {}
end
...
-- =====================================
-- HELPER: Apply dark MythicHub backdrop
-- =====================================
...
local scanTip = CreateFrame("GameTooltip", "MythicHubItemScanTip", nil, "GameTooltipTemplate")
...
local function GetUpgradeTrackFromTooltip(slotID, unit)
    ...
    for i = 2, scanTip:NumLines() do
        local left = _G["MythicHubItemScanTipTextLeft" .. i]
        if left then
...
local function SkinCharacterFrame()
    ...
    do
        local mplusFrame = CreateFrame("Button", "MythicHub_MythicScoreWidget", CharacterFrame)
        ...
        accent:SetColorTexture(0.00, 0.686, 1.00, 0.60)
        ...
        mplusFrame:SetScript("OnClick", function()
            if MythicHub_MythicHub then
                MythicHub_MythicHub:Toggle()
            end
        end)
        ...
        mplusFrame:SetScript("OnEnter", function(self)
            ...
            GameTooltip:SetText("Mythic+ Score", 0.00, 0.686, 1.00)
            local score = 0
            if C_ChallengeMode and C_ChallengeMode.GetOverallDungeonScore then
                score = C_ChallengeMode.GetOverallDungeonScore() or 0
            end
...
-- Convention MythicHub : on capture la position avec GetLeft()/GetBottom().
...
function CS.MakeCharacterMovable()
    ...
    if not charMovableHooked then
        ...
        CharacterFrame:SetScript("OnDragStop", function(self)
            ...
            -- Capture position via GetLeft()/GetBottom() (convention MythicHub)
...
function CS.ApplySettings()
    if isInitialized then
        ...
        print("|cff00AFFFMythicHub|r " .. (L["msg_char_skin_reload"] or "Character Skin: /reload to apply changes."))
    end
end
