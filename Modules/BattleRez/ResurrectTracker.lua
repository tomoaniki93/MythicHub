...
-- Loaded from RaidFrame.xml (so MythicHub_PartyFrames AND MythicHub_RaidFrames both
-- exist) and initialized after RaidFrames in Init.lua.
-- =====================================

MythicHub_ResurrectTracker = MythicHub_ResurrectTracker or {}
local RT = MythicHub_ResurrectTracker
...
local COLORS = {
    accent   = { 0.000, 0.686, 1.000, 1 },    -- Teal (primary)
    ...
    border   = { 0.000, 0.686, 1.000, 0.9 },  -- Teal border
...
local function DB()
    return MythicHubDB and MythicHubDB.battleRez
end
...
local function CreateBrezFrame()
    ...
    brezFrame = CreateFrame("Frame", "MythicHub_BattleRezCounter", UIParent, "BackdropTemplate")
    ...
    dl:SetText((MythicHub_L and MythicHub_L["mover_battlerez"]) or "Battle Rez")
    dl:Hide()
...
function RT.UpdateResurrect()
    local PF = MythicHub_PartyFrames
    if PF and PF.frames then
        local pdb = MythicHubDB and MythicHubDB.partyFrames
        for _, f in pairs(PF.frames) do
            UpdateOneFrame(f, pdb)
        end
    end
    local RF = MythicHub_RaidFrames
    if RF and RF.frames then
        local rdb = MythicHubDB and MythicHubDB.raidFrames
...
function RT.Initialize()
    ...
    if MythicHub_Movers and MythicHub_Movers.RegisterEntry then
        MythicHub_Movers.RegisterEntry({
            label    = (MythicHub_L and MythicHub_L["mover_battlerez"]) or "Battle Rez",
            ...
            isActive = function()
                return MythicHubDB and MythicHubDB.battleRez and MythicHubDB.battleRez.enabled
            end,
        })
    end
