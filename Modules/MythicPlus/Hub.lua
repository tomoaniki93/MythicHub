...
MythicHub_MythicHub = MythicHub_MythicHub or {}
local HUB = MythicHub_MythicHub

-- ═══════════════════════════════════════════════════════════════════════
--  THEME  (ObjectiveTracker / MythicHub dark panel)
-- ═══════════════════════════════════════════════════════════════════════
local C = {
...
function HUB:Build()
    ...
    -- ── Main Frame ───────────────────────────────────────────────────
    local F = CreateFrame("Frame", "MythicHub_OverviewFrame", UIParent, "BackdropTemplate")
    self.Frame = F
    ...
    F:SetScript("OnDragStart", function(s)
        local db = MythicHubDB and MythicHubDB.mythicHub
        if db and db.locked == false then s:StartMoving() end
    end)
    F:SetScript("OnDragStop", function(s)
        ...
        if db then
            ...
            db.position = { anchor = point or "CENTER", relTo = relPoint or "CENTER", x = x or 0, y = y or 0 }
        end
    end)
    F:Hide()
    ...
    MythicHub_Utils.CloseOnEscape(_G["MythicHub_OverviewFrame"])
...
function HUB:Toggle()
    if MythicHubDB and MythicHubDB.mythicHub and MythicHubDB.mythicHub.enabled == false then return end
