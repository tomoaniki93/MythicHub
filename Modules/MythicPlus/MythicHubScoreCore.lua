-- =====================================================================
-- MythicHubScoreCore.lua — MythicHubScore integrated into MythicHub (MythicPlus)
-- Addon table, color palette, utilities.
-- =====================================================================

local L = MythicHub_L

MythicHub_MythicHubScore = MythicHub_MythicHubScore or MythicHub_TomoScore or {}
-- Backward-compatible runtime alias for older integrations during the beta.
MythicHub_TomoScore = MythicHub_MythicHubScore
local TS = MythicHub_MythicHubScore

-- ── Color palette ─────────────────────────────────────────────────────────
-- Derived from the shared MythicHub theme instead of the standalone dark/teal
...
-- Theme lives in the MythicHub_Options load-on-demand sub-addon. The previous
...
local T = setmetatable({}, { __index = function(_, key)
    local theme = MythicHub_Widgets and MythicHub_Widgets.Theme
...
TS.C = {
    ...
    ACCENT        = tint(T.accent,      { 0.00,  0.686, 1.00  }, 1.00),
    ACCENT_DIM    = tint(T.accentDark,  { 0.00,  0.36,  0.62  }, 0.60),
    ...
    BORDER_ACCENT = tint(T.accent,      { 0.00,  0.686, 1.00  }, 0.80),
    BAR_ACCENT    = tint(T.accent,      { 0.00,  0.686, 1.00  }, 0.80),
    ...
    TEXT_ACCENT   = tint(T.textHeader,  { 0.00,  0.686, 1.00  }, 1.00),
...
-- ── DB access ─────────────────────────────────────────────────────────────────
function TS:GetDB()
    return MythicHubDB and MythicHubDB.MythicHubScore
end
