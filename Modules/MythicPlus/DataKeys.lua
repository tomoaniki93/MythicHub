...
MythicHub_DataKeys = MythicHub_DataKeys or {}
...
function MythicHub_DataKeys.RefreshFromAPI()
...
function MythicHub_DataKeys.IsAPIReady()
    return apiDataReady
end
...
function MythicHub_DataKeys.GetDungeonName(mapID)
...
local function ResolveEntry(mapID)
    local entry = DB[mapID]
    local live  = MythicHub_DataKeys.GetDungeonName(mapID)
...
MythicHub_DataKeys.ResolveEntry = ResolveEntry

function MythicHub_DataKeys.GetShortName(mapID)
    if not mapID then return nil end
    mapID = tonumber(mapID)
    if not mapID then return nil end
...
function MythicHub_DataKeys.GetDisplayName(mapID)
    return MythicHub_DataKeys.GetDungeonName(mapID)
        or MythicHub_DataKeys.GetShortName(mapID)
        or ("ID:" .. tostring(mapID))
end

--- Get teleport spell ID for a dungeon
function MythicHub_DataKeys.GetTeleportSpellID(mapID)
    if not mapID then return nil end
    mapID = tonumber(mapID)
    if not mapID then return nil end
    local entry = ResolveEntry(mapID)
...
function MythicHub_DataKeys.GetMapIDByName(name)
...
function MythicHub_DataKeys.GetEntry(mapID)
    if not mapID then return nil end
    mapID = tonumber(mapID)
    if not mapID then return nil end
...
function MythicHub_DataKeys.GetAllMapIDs()
    local ids = {}
    local seen = {}
    for id in pairs(DB) do
        ids[#ids + 1] = id
...
function MythicHub_DataKeys.IsTeleportKnown(spellID)
...
function MythicHub_DataKeys.GetCurrentSeasonIDs()
...
MythicHub_DataKeys.CURRENT_SEASON = HARDCODED_SEASON

--- Get current season dungeon list with full data
--- Dynamically resolves names for API-discovered dungeons
function MythicHub_DataKeys.GetCurrentSeasonData()
    local seasonIDs = MythicHub_DataKeys.GetCurrentSeasonIDs()
    local result = {}
    ...
    for _, mapID in ipairs(seasonIDs) do
        ...
        if entry then
            ...
            local name = MythicHub_DataKeys.GetDungeonName(mapID) or ("ID:" .. mapID)
            local short = MythicHub_DataKeys.GetShortName(mapID) or name
...
refreshFrame:SetScript("OnEvent", function(_, event)
    ...
    MythicHub_DataKeys.RefreshFromAPI()
    ...
    if event == "PLAYER_ENTERING_WORLD" and refreshCount == 0 then
        ...
        C_Timer.After(10, function()
            ...
            local ids = MythicHub_DataKeys.GetCurrentSeasonIDs()
            if ids and #ids > 0 then
                MythicHub_DataKeys.CURRENT_SEASON = ids
            end
        end)
    end
end)
