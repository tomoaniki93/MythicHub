...
-- Keystone data comes from KeySync.lua, MythicHub's own sharing module.
-- /tm key  -> list party keystones in group chat
-- /tmt kr  -> keystone roulette UI with random pick
-- =====================================================================

local L = MythicHub_L
-- Keystone data now comes from MythicHub's own sync module (KeySync.lua),
-- which exposes the same four functions this file already used.
local openRaidLib = MythicHub_KeySync

local PREFIX = "|cff00AFFFTomo|r|cFFFFFFFFMod|r"

-- Keep a namespaced global + local alias for backward compatibility (/tm key)
MythicHub_MythicKeys = MythicHub_MythicKeys or {}
local MK = MythicHub_MythicKeys
MK.enabled = false
MK.keyData = {}

-- Also expose as MythicHub_MythicPartyKeys for the MythicTracker /tmt integration
MythicHub_MythicPartyKeys = MK
...
local function GetSettings()
    if not MythicHubDB or not MythicHubDB.MythicKeys then return nil end
    return MythicHubDB.MythicKeys
end
...
local function GetDungeonShortName(mapID)
    if MythicHub_DataKeys then return MythicHub_DataKeys.GetShortName(mapID) end
    local name = C_ChallengeMode.GetMapUIInfo(mapID)
    return name and name:sub(1, 4):upper() or "???"
end

local function GetDungeonFullName(mapID)
    if MythicHub_DataKeys then return MythicHub_DataKeys.GetDungeonName(mapID) end
    local name = C_ChallengeMode.GetMapUIInfo(mapID)
    return name or "???"
end
    ...
    for i = 1, numMembers - 1 do
        ...
        if uName then
            ...
            if info and info.level and info.level > 0 then
                ...
                local class = MythicHub_Utils and MythicHub_Utils.UnitClassToken(unit)
                results[#results + 1] = {
                    name  = uName,
                    class = class,
                    unit  = unit,
...
function MK:SendKeysToChat()
    if MythicHubDB and MythicHubDB.MythicKeys and MythicHubDB.MythicKeys.enabled == false then return end
    ...
    SendChatMessage("-- MythicHub - Party Keys --", channel)
...
local C_TMT = MythicHub_MythicTracker and MythicHub_MythicTracker.C or {
...
local function EnsureRouletteFrame()
    ...
    local F = CreateFrame("Frame", "MythicHub_KeyRoulette", UIParent, "BackdropTemplate")
    RouletteFrame = F
    ...
    F.title:SetText("|cff00AFFFTomo|r|cFFFFFFFFMod|r  |cFFAAAAAAKeystone Roulette|r")
...
local function DoSpin()
    ...
    local function Tick()
        ...
        if step >= totalSteps then
            ...
            if IsInGroup() then
                local channel = IsInGroup(LE_PARTY_CATEGORY_INSTANCE) and "INSTANCE_CHAT" or "PARTY"
                local msg = string.format("[MythicHub Roulette] >> %s +%d (%s) <<",
                    dungeonName, w.level, w.name)
                SendChatMessage(msg, channel)
            end
            return
...
if openRaidLib then
    openRaidLib.RegisterCallback("MythicHub_MythicKeys", "KeystoneUpdate", function()
...
MythicHub_RegisterModule("MythicKeys", MK)
