...
-- ---------------------------------------------------------------------
local function DB()
    if type(MythicHubDB) ~= "table" then return nil end
    if type(MythicHubDB.Keystones) ~= "table" then MythicHubDB.Keystones = {} end
    return MythicHubDB.Keystones
end
...
local function Fire(fullName)
    for _, cb in ipairs(callbacks) do
        ...
        if not ok then
            ...
            print("|cff00AFFFMythicHub|r KeySync: callback error (" .. tostring(cb.owner) .. "): " .. tostring(err))
        end
    end
end
...
function KS.Broadcast(force)
    if MythicHubDB and MythicHubDB.KeySync and MythicHubDB.KeySync.enabled == false then return end
...
-- ---------------------------------------------------------------------
function KS.Debug()
    print("|cff00AFFFMythicHub|r KeySync: prefix " .. PREFIX
    ...
    for name, entry in pairs(KS.Data) do
        count = count + 1
        print(string.format("  |cff00AFFF%s|r  +%d  map %d", name,
            entry.level or 0, entry.challengeMapID or 0))
    end
    print("|cff00AFFFMythicHub|r KeySync: " .. count .. " entries stored.")
    ...
    print("|cff00AFFFMythicHub|r KeySync: request sent, re-run in a second.")
end
