...
local L = MythicHub_L
local TMT = {}
MythicHub_MythicTracker = TMT

-- ═══════════════════════════════════════════════════════════════════════
--  COLOR PALETTE — derived from the MythicHub brand tokens
...
function TMT:BuildPalette()
    ...
    local TH = MythicHub_Widgets and MythicHub_Widgets.Theme

    local BRAND = (U and U.BRAND)       or { 0.000, 0.686, 1.000 }
    local DARK  = (U and U.BRAND_DARK)  or { 0.000, 0.360, 0.620 }
    local HOVER = (U and U.BRAND_HOVER) or { 0.250, 0.820, 1.000 }
    ...
    -- Surfaces: near-black with a mint cast so the panel reads as MythicHub
    -- rather than as a generic dark box.
    C.BG         = { 0.025, 0.040, 0.065, 0.92 }
    C.BG_HEADER  = { 0.035, 0.070, 0.110, 1.00 }
    C.BG_ROW_ALT = { 0.035, 0.070, 0.110, 0.45 }
    C.BAR_TRACK  = { 0.045, 0.065, 0.095, 1.00 }
    ...
    C.HEX_BRAND = (U and U.BRAND_HEX) or "00AFFF"
    C.HEX_RED   = ToHex(red)
...
TMT.FONT_HUD   = UNIT_NAME_FONT or STANDARD_TEXT_FONT or "Fonts\\FRIZQT__.TTF"
...
local function GetDB()
    return MythicHubDB and MythicHubDB.MythicTracker
end
...
function TMT:BuildFrame()
    ...
    local W = self.W
    local F = CreateFrame("Frame", "MythicHub_MythicTrackerFrame", UIParent)
    self.Frame = F
...
function TMT:ResetPosition()
    local def = MythicHub_Defaults.MythicTracker.position
...
-- Slash commands are centralized in Core/Slash.lua for standalone MythicHub.
